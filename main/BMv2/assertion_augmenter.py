...
def update_metadata_name(code, meta_name, cnt_blk):
	pat = re.compile(r'control[\s]+{}[\s]*\('.format(cnt_blk))
	#使用正则表达式模式 pat 在代码中搜索控制块名称的位置，并将结果存储在 ind_cnt_blk 变量中
	ind_cnt_blk = re.search(pat, code)
...
def assert_augmenter(p4_file, assert_list, cnt_blk, meta, header):
	...
	for asser in assert_list:
		with open(file_path+asser,'r') as a:
			a_code = a.read()
			#
			for i,j in zip([m.start() for m in re.finditer(BEGIN_FILTER_PATH, a_code)],\
				  [m.start() for m in re.finditer(END_FILTER_PATH, a_code)]):
...
if len(sys.argv) != 5 and len(sys.argv) != 4:
	...
	#输入文件有p4_file,control_block,meta_name，
	#可能还会有header
...
if file_path != "":
	file_path = file_path+'/'
	print ("\n\tPATH : ",'/'+file_path)
	#list_of_files 变量将包含目录 file_path 中所有文件和子目录的名称列表（找assertions）。
	list_of_files = os.listdir(file_path)

#路径空的话，那么就搜索当前目录（找assertions）
else:
	file_path = ""
...
# print("###"+str(sorted_list_of_files))

#p4_file：p4文件，sorted_list_of_files中的逆序，control_block,meta_name元数据名称,header
